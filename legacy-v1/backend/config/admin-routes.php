...
use App\Controllers\Admin\AdminAgencyController;
...
return function (App $app) {
    ...
    $app->group('/admin/api', function (RouteCollectorProxy $group) {
        ...
        $group->group('/helpers', function (RouteCollectorProxy $helpers) {
            ...
            $helpers->put('/{id}/verify-physical', [AdminHelperController::class, 'verifyPhysical'])
                ->add(new PermissionMiddleware('helpers.verify'));
        ...
        $group->group('/agencies', function (RouteCollectorProxy $agencies) {
            $agencies->get('', [AdminAgencyController::class, 'getAllAgencies'])
                ->add(new PermissionMiddleware('agencies.view'));
            $agencies->get('/{id}', [AdminAgencyController::class, 'getAgencyDetails'])
                ->add(new PermissionMiddleware('agencies.view'));
            $agencies->put('/{id}/verify', [AdminAgencyController::class, 'verifyAgency'])
                ->add(new PermissionMiddleware('agencies.edit'));
            $agencies->delete('/{id}', [AdminAgencyController::class, 'deleteAgency'])
                ->add(new PermissionMiddleware('agencies.delete'));
        });
