...
class VerificationController
{
    ...
    public function preVerifyNin(Request $request, Response $response): Response
    {
        $data = $request->getParsedBody();

        $ninNumber = $data['nin'] ?? $data['nin_number'] ?? '';
        ...
        if (!$ninNumber || !$firstName || !$lastName) {
            return $this->jsonResponse($response, [
                'success' => false,
                'error' => 'NIN number, first name, and last name are required'
            ], 400);
        }
        ...
        $result = $this->verificationService->preMatchNin($ninNumber, $firstName, $lastName);

        if ($result['success']) {
            return $this->jsonResponse($response, [
                'success' => true,
                'response' => 'Success', // Match frontend expectation
            ...
            ], 400); // 400 to fail the try/catch in frontend if needed, or 200 with success=false. We'll use 400 to match error handling.
        }
    }
