...
class Migration
{
    ...
    private function getMigrations(): array
    {
        return [
            ...
            '023_align_payments_table',
            '024_add_guarantor_to_helpers',
            '025_add_commission_settings',
            '026_create_standalone_verifications_table',
            '027_add_nin_fee_settings',
        ];
    }
    ...
    private function migrate_024_add_guarantor_to_helpers(): void
    {
        $columns = [
            ...
            'guarantor_nin VARCHAR(20)',
            'emergency_contact_name VARCHAR(255)',
            'emergency_contact_phone VARCHAR(20)',
            'physical_verification_status VARCHAR(20) DEFAULT \'pending\'',
    ...
    private function migrate_025_add_commission_settings(): void
    {
        $stmt = $this->pdo->prepare("INSERT OR IGNORE INTO settings (key_name, value, category) VALUES (?, ?, ?)");
        $stmt->execute(['commission_type', '"percentage"', 'payments']);
        $stmt->execute(['commission_fixed_amount', '5000', 'payments']);
    }
    private function migrate_023_align_payments_table(): void
    {
        $this->pdo->beginTransaction();
        try {
            if ($this->isSqlite) {
                // SQLite table recreation
                $this->pdo->exec("CREATE TABLE payments_new (
                ...
                // Check which columns exist in the current payments table to be safe
                // (though we know 015 and 020 added them)
                $this->pdo->exec("INSERT INTO payments_new (id, booking_id, amount, currency, gateway, tx_ref, gateway_ref, status, payment_method, metadata, paid_at, created_at, updated_at, payment_type, commission_amount, helper_amount, checkout_url)
                                 SELECT id, booking_id, amount, currency, gateway, tx_ref, gateway_ref, status, payment_method, metadata, paid_at, created_at, updated_at, 
                                 COALESCE(payment_type, 'service_fee'), COALESCE(commission_amount, 0), COALESCE(helper_amount, 0), checkout_url FROM payments");

                $this->pdo->exec("DROP TABLE payments");
                $this->pdo->exec("ALTER TABLE payments_new RENAME TO payments");
                
                $this->pdo->exec("CREATE INDEX IF NOT EXISTS idx_payments_booking_id ON payments(booking_id)");
                $this->pdo->exec("CREATE INDEX IF NOT EXISTS idx_payments_service_request_id ON payments(service_request_id)");
                $this->pdo->exec("CREATE INDEX IF NOT EXISTS idx_payments_tx_ref ON payments(tx_ref)");
                $this->pdo->exec("CREATE INDEX IF NOT EXISTS idx_payments_status ON payments(status)");
            } else {
                // MySQL logic
                $this->pdo->exec("ALTER TABLE payments MODIFY COLUMN booking_id INTEGER NULL");
                $this->pdo->exec("ALTER TABLE payments ADD COLUMN service_request_id INTEGER NULL AFTER booking_id");
                $this->pdo->exec("ALTER TABLE payments ADD CONSTRAINT fk_payments_service_request FOREIGN KEY (service_request_id) REFERENCES service_requests(id) ON DELETE SET NULL");
            }
            $this->pdo->commit();
        } catch (\Exception $e) {
            $this->pdo->rollBack();
    ...
    private function migrate_026_create_standalone_verifications_table(): void
    {
        $sql = "CREATE TABLE IF NOT EXISTS standalone_verifications (
            id INTEGER PRIMARY KEY {$this->autoIncrement()},
            reference VARCHAR(50) NOT NULL UNIQUE,
            email VARCHAR(255) NOT NULL,
            requested_nin VARCHAR(20) NOT NULL,
            first_name VARCHAR(255) NOT NULL,
            last_name VARCHAR(255) NOT NULL,
            status VARCHAR(20) DEFAULT 'pending', -- pending, paid, verified, failed
            match_result INTEGER DEFAULT NULL,    -- 1 (match), 0 (mismatch)
            qoreid_response TEXT,                 -- json
        ...
        $this->pdo->exec("CREATE INDEX IF NOT EXISTS idx_st_veri_ref ON standalone_verifications(reference)");
        $this->pdo->exec("CREATE INDEX IF NOT EXISTS idx_st_veri_email ON standalone_verifications(email)");
    }

    private function migrate_027_add_nin_fee_settings(): void
    {
        $stmt = $this->pdo->prepare("INSERT OR IGNORE INTO settings (key_name, value, category) VALUES (?, ?, ?)");
        $stmt->execute(['nin_verification_fee', '{"amount": 5000, "currency": "NGN"}', 'payments']);
    }
}
