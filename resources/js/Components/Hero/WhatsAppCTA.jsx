 ...
 * one styling source of truth, N attribution points via `source`.
 * WhatsApp green #25D366 is the single allowed brand-palette break.
 */
...
const SIZES = {
    ...
    lg: 'w-full sm:w-auto px-8 text-base gap-3 h-14', // 56px tall — hero primary
};
...
export default function WhatsAppCTA({
    ...
    badge = null, // e.g. 'We reply in minutes, 7am–10pm'
    className = '',
    glyphClass = 'w-5 h-5',
}) {
    ...
    const button = (
        <a
            ...
            onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = WA_GREEN_DARK)}
            onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = WA_GREEN)}
        >
            <WhatsAppGlyph className={glyphClass} />
            <span>{label}</span>
        </a>
    );
    ...
    return (
        <div className="flex flex-col items-center gap-2">
            {button}
            <p className="text-[13px] text-white/85 tracking-wide">{badge}</p>
        </div>
    );
}
