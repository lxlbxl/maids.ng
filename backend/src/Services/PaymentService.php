...
class PaymentService
{
    ...
    public function createPayment(int $bookingId, string $gateway = 'flutterwave', string $type = 'service_fee'): array
    {
        ...
        if ($type === 'salary') {
            ...
            $stmt = $this->pdo->prepare("SELECT value FROM settings WHERE key_name = 'commission_percent'");
            $stmt->execute();
            $commissionConfig = $stmt->fetch();
            $commissionPercent = (int) ($commissionConfig['value'] ?? 10);
            ...
            $helperAmount = $salary - $commission;
            $amount = $salary;
        ...
        $stmt = $this->pdo->prepare("
            INSERT INTO payments (booking_id, amount, currency, gateway, tx_ref, status, payment_type, commission_amount, helper_amount)
            VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?)
        ");
        $stmt->execute([
            $bookingId,
        ...
        if ($result) {
            ...
            $stmt = $this->pdo->prepare("
                UPDATE bookings SET
                    status = 'confirmed',
                    updated_at = CURRENT_TIMESTAMP
                WHERE id = (SELECT booking_id FROM payments WHERE tx_ref = ?)
            ");
            $stmt->execute([$txRef]);
    ...
    {
        $stmt = $this->pdo->prepare("
            SELECT p.*, b.employer_id, b.helper_id, b.reference as booking_ref
            FROM payments p
